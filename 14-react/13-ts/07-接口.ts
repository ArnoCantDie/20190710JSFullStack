// ts 中语法
// 数据类型：number,string,boolean,array,元组,enum,null,undefiend,void,never,any

// 函数  给形参类型   给函数返回值类型

// 类  属性+方法   实例属性和静态属性   实例方法和静态方法   构造器   访问器  设置器
// 抽象类  让子来继承  不能new  抽象方法
// 继承  super()--->父类的constructor

// 接口(和java中一样) 比抽象类还抽象  里面的方法都是抽象  子类可以继承接口  通常子类是去实现接口，需要实现接口中的抽象方法
...
// }
// ------------------------- 接口对函数进行校验
// 现在你可以定义一个接口对sum函数进行检验
// 这里面约束的仅仅是形参列表和函数的返回值
// interface MySum{
//     // 只需要写形参列表和返回值类型
//     // 形参名是不固定的
//     (n:number):number
...
// ------------------------- 可索引的接口(约束数组)
// 下面是约束数组的两种方法
// let arr:string[] = ["z3","l4"];
// let arr1:Array<string> = ["abc","def"];

// 可以利用接口对 数组 进行约束 
// interface ArrInterfase{
//     // [index:number]  索引是Number类型
//     // string 索引对应的值是string
//     [index:number]:string
// }
// let arr:ArrInterfase = ["z3","l4"];
// let arr1:ArrInterfase = [1,2,3];
// ------------------------- 可索引的接口(约束对象)
